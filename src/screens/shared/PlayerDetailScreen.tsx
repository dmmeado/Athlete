...
type PlayerDetailScreenProps = {
  navigation: NativeStackNavigationProp<CoachSearchStackParamList, 'PlayerDetail'>;
  route: RouteProp<CoachSearchStackParamList, 'PlayerDetail'>;
};

export default function PlayerDetailScreen({ navigation, route }: PlayerDetailScreenProps) {
  ...
  const handleContact = async () => {
    ...
    try {
      // For now, show a message that messaging is coming soon
      Alert.alert(
        'Contact Player',
        'Messaging functionality will be available soon. You can reach out to players through the Messages tab.'
      );
    } catch (error) {
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.contactContainer}>
        <Button title="Contact Player's Family" onPress={handleContact} />
      </View>
      ...
      {player.highlightVideos.length > 0 && (
        <View style={styles.section}>
          ...
          <Text style={styles.emptyText}>
            {player.highlightVideos.length} video(s) available
          </Text>
        </View>
      )}
